...
def connect_to_server(server_name):
    ...
    ip_address = socket.gethostbyname(server_name)    # Utilizamos la función gethostbyname de la clase socket para sacar la ip. Esta función recibe un string, el cual es la URL de una pagina
    ...
    # Crear socket
    # COMPLETAR ABAJO DE ESTA LINEA
    big_real_socket = socket.socket(socket.AF_INET,socket.SOCK_STREAM) # Creamos un socket de tipo IPv4 y que sea de STREAM puesto que queremos descargar la pagina de forma estable y segura
    big_real_socket.connect((ip_address,80))    # Nos conectamos al servidor, para ello usamos socket.connect, la cual espera como parametro una tupla que contenga (Server_ip,Puerto a utilizar)
    # Aqui deben conectarse al puerto correcto del servidor
    # NO MODIFICAR POR FUERA DE ESTA FUNCION
    return big_real_socket
